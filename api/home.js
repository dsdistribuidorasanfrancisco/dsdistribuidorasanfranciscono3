// api/home.js — Tienda No.3 (template3). Sirve la portada con meta tags
// Open Graph actualizados automáticamente desde Firebase (nombre, descripción y logo).
// Lee el HTML base desde template3.html (en la raíz del proyecto).

const fs = require("fs");
const path = require("path");
const crypto = require("crypto");

const STORE_NAME_DEFAULT = "DS Distribuidora San Francisco No.3";
...
const FIREBASE_URL =
  "https://dsdistribuidorasfc3-default-rtdb.firebaseio.com/settings.json";

let templateCache = null;
function loadTemplate() {
  if (templateCache) return templateCache;
  // Nombre del archivo HTML: debe coincidir EXACTO con el del repo y con vercel.json.
  templateCache = fs.readFileSync(path.join(process.cwd(), "template3.html"), "utf8");
...
module.exports = async (req, res) => {
  ...
  <!-- OG_META_END -->`;
    ...
    // MODO DEBUG TEMPORAL: muestra el error real en pantalla en vez del 500 genérico.
    // Quita este bloque catch exterior una vez resuelto el problema.
    res.setHeader("Content-Type", "text/plain; charset=utf-8");
    res.status(500).send("ERROR EN /api/home:\n\n" + (err && err.stack ? err.stack : String(err)));
  }
};
